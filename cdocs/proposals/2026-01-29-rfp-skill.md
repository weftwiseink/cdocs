...
> BLUF(mjr/cdocs/rfp): A `/cdocs:rfp` skill that scaffolds lightweight `request_for_proposal` stubs (`type: proposal` in `cdocs/proposals/`) containing a topic and enough context for a future author to elaborate into a full proposal.
> RFP stubs have four required sections (BLUF, Objective, Scope, Open Questions) and optional freeform sections.
...
> Tags are author-supplied at creation; the triage subagent handles tag refinement post-authoring.
> No special hook validation is needed: the existing PostToolUse frontmatter check covers RFP stubs identically to full proposals.
...
- **Triage subagent** (proposed in `haiku-subagent-workflow-automation.md`): handles post-authoring tag refinement as a triage responsibility, not an authoring responsibility.
...
2. Propose reads the stub, detects `status: request_for_proposal`, and switches to elaboration mode.
3. Propose preserves the original `first_authored` block and fills in the full section set (Background, Proposed Solution, Design Decisions, Edge Cases, Phases) around the existing BLUF, Objective, and Scope content.
4. Status transitions from `request_for_proposal` to `wip`.

> NOTE(claude-opus-4-5/cdocs/rfp-skill): The propose skill's SKILL.md currently assumes creating a new file.
> Phase 2 of implementation adds the "elaborate existing RFP" path to propose's invocation logic.
...
### Decision 7: Skill name `rfp` despite acronym convention break

**Decision:** Name the skill `rfp`, not `request` or another verb.
...
While other skills use verbs (propose, review, report, implement) or nouns (devlog, status, init), `rfp` is concise, unambiguous, and already in the project vocabulary.
`request` would be more consistent with the verb convention but less immediately clear about what's being created.
...
### 3. User invokes `/cdocs:propose` on an RFP without knowing it's a stub

If the user runs `/cdocs:propose cdocs/proposals/YYYY-MM-DD-topic.md` on a full proposal (not an RFP), the propose skill should detect `status: wip` or later and warn that the document is already a full proposal, suggesting review or revision instead.
...
6. **Propose elaboration**: create an RFP stub, then run `/cdocs:propose` on it, verify the stub is elaborated in-place with full sections and `status: wip`.
7. **Collision handling**: create a stub, then run `/cdocs:rfp` with the same topic on the same date, verify collision detection.
...
3. Test: invoke `/cdocs:rfp test-topic`, verify output file and frontmatter.
...
1. Update `plugins/cdocs/skills/propose/SKILL.md` to handle an existing RFP stub as input.
2. Add invocation path: if `$ARGUMENTS` is a path to a file with `status: request_for_proposal`, elaborate in-place instead of creating a new file.
3. Preserve `first_authored`, transition `status: request_for_proposal -> wip`, fill in full section set around existing content.
4. Test: create an RFP stub, then `/cdocs:propose` on it, verify in-place elaboration.
