...
Option C loses `/cdocs:` namespacing (skills become `/devlog`, `/propose`, etc.) but works without plugin infrastructure.
...
- `/cdocs:devlog`: Most commonly auto-invoked by Claude when starting substantive work (triggered by the "always create a devlog" rule). User can also invoke directly.
- `/cdocs:propose`: Typically user-invoked when a design needs specification. Claude may suggest it when scoping complex work.
- `/cdocs:review`: Typically user-invoked. Claude may suggest it when a document reaches `review_ready`.
- `/cdocs:report`: Typically user-invoked after research or analysis. Claude may suggest it after completing investigative work.
- `/cdocs:status`: User-invoked for doc inventory. Claude may auto-invoke to check document state.
- `/cdocs:init`: User-invoked to scaffold a new project.

#### `/cdocs:devlog`: Create a development log

- **User-invocable:** yes
- **Argument hint:** `[feature_name]`
- **Expected usage:** Most commonly auto-invoked by Claude. The `rules/writing-conventions.md` rule contains the "always create a devlog when starting substantive work" directive. This rule is the *trigger*; the skill is the *mechanism*. The user can also invoke `/cdocs:devlog feature-name` directly.
- **Behavior:**
...
#### `/cdocs:propose`: Author a proposal
...
#### `/cdocs:review`: Review a document
...
#### `/cdocs:report`: Generate a report
...
#### `/cdocs:status`: Query and manage documents
...
#### `/cdocs:init`: Scaffold CDocs in a project

- **User-invocable:** yes
- **Argument hint:** `[--minimal]`
- **Behavior:**
  1. Create directory structure: `cdocs/{devlogs,proposals,reviews,reports,_media}/`.
  2. Generate lightweight READMEs in each subdir with format summaries and a reference to the full skill (e.g., "See `/cdocs:propose` for complete authoring guidelines"). These serve as fallback documentation for non-plugin users and GitHub readers.
...
- **Success criteria:** `/cdocs:devlog my_feature` creates a well-formed devlog. `/cdocs:propose my_topic` creates a well-formed proposal. Starting substantive work without explicit invocation triggers devlog auto-creation.
