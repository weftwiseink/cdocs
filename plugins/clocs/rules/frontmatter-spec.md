...
# CLocs Frontmatter Specification

Every CLocs document requires YAML frontmatter.
...
```yaml
---
review_of?: clocs/.../YYYY-MM-DD-doc-name.md   # reviews only
first_authored:
...
Use `/` namespacing to capture workstream hierarchy: `organization/initial-scaffolding`, `clocs/plugin-architecture/core-skills`.

### `type` (required)
Corresponds to subdirectory under `clocs/`:
- `devlog`: Development logs in `clocs/devlogs/`
- `proposal`: Design proposals in `clocs/proposals/`
- `review`: Document reviews in `clocs/reviews/`
- `report`: Reports in `clocs/reports/`
...
All clocs use `{YYYY-MM-DD}-{dash-case}.md` format.
Examples:
- `2026-01-29-clocs-plugin-architecture.md`
- `2026-01-29-review-of-clocs-plugin-architecture.md`

## Media

Media files are dated, saved to `clocs/_media/`, and embedded into the relevant doc.
