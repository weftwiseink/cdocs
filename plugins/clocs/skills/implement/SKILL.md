...
```
/cloc:implement clocs/proposals/2026-01-29-topic.md
```
...
/cloc:implement
```
Scan `clocs/proposals/` for documents with `status: implementation_ready`.
Present the list to the user and ask which proposal to implement.
If no proposals are `implementation_ready`, report that and suggest checking `/cloc:status --type=proposal`.
...
3. **Create a devlog**: invoke `/cloc:devlog` for the implementation session.
...
### Use cloc skills as appropriate
- `/cloc:review` when implementation is complete and ready for evaluation.
- `/cloc:report` if the implementation reveals findings worth documenting separately.

### Note deviations from the proposal
- If the implementation diverges from the proposal's design, document why in the devlog.
- Use `> NOTE(author/workstream):` callouts in the devlog for deviations (ie `> NOTE(opus/clocs/haiku_subagent)`).
...
- Go through subagent `/cloc:review` loops until accepted or escalation needed
