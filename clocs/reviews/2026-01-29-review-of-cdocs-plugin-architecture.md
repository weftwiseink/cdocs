---
review_of: clocs/proposals/2026-01-29-clocs-plugin-architecture.md
first_authored:
  by: "@claude-opus-4-5-20251101"
  at: 2026-01-29T12:30:00-08:00
task_list: clocs/plugin-architecture
type: review
state: live
...
# Review: CLocs Plugin Architecture

## Summary Assessment

The proposal provides a reasonable mapping of CLocs concerns onto Claude Code extension mechanisms and correctly identifies plugins as the distribution unit.
...
The proposal treats `/cloc:devlog` identically to `/cloc:proposal`, a user-invoked command that creates a document.
...
- **Devlog:** The user requests *work*. "Fix the auth bug." The devlog is infrastructure that Claude creates as a side effect of doing the work. The user should not need to invoke `/cloc:devlog` - it should happen automatically.
...
3. Consider whether the skill should default to `disable-model-invocation: false` + document that Claude auto-invokes it, rather than presenting it as a peer of `/cloc:proposal`.
...
The proposal says "any project can adopt CLocs by enabling the plugin and running `/cloc:init`" but never explains how a project gets the plugin.
...
3. **Global install** (`~/.claude/plugins/clocs/`): not mentioned.
...
Removing the READMEs from `clocs/devlogs/` and `clocs/proposals/` has a cost the proposal doesn't address: discoverability without the plugin.

Scenarios where this matters:
- A team member uses Claude without the CLocs plugin enabled.
- A developer reads the `clocs/` directory without Claude at all (e.g., on GitHub).
...
- Keep lightweight READMEs that reference the skill for full guidance (e.g., "See `/cloc:devlog` skill for complete guidelines").
- Have the `init` skill generate format-guide READMEs in the project's `clocs/` subdirs.
- Accept this as a deliberate tradeoff and state it explicitly.

### Status skill: context cost unaddressed

The status skill scans all `clocs/**/*.md` files using Glob+Read, parsing frontmatter from each.
...
- Propose a frontmatter index file (e.g., `clocs/.index.json`) maintained by hooks.
- Move the MCP server timeline up for the status use case specifically.

### Hook script: missing implementation details

The proposal references `clocs-validate-frontmatter.sh` but doesn't address:
- Where the script lives in the plugin directory (presumably `hooks/`).
- How the hook command resolves the script path (needs `${CLAUDE_PLUGIN_ROOT}/hooks/clocs-validate-frontmatter.sh`).
...
- `frontmatter-spec.md` should arguably only apply when working on clocs files: `paths: ["clocs/**/*.md"]`.

If both are unscoped, the frontmatter spec pollutes Claude's context when working on non-clocs files.
If the writing conventions are scoped to clocs, they don't influence general communication.
...
The layout shows `clocs/` (dogfooding) alongside `skills/` (plugin source) at the repo root.
The proposal claims this "works naturally" but doesn't explain *why*: plugin components are discovered from specific paths (`skills/`, `hooks/`, `rules/`), and `clocs/` is not a plugin discovery path.
A sentence of explanation would prevent confusion.

### Edge case #5 is wrong

> "Rule conflicts with project CLAUDE.md: If a project has its own writing conventions that conflict with CLocs rules, the project's rules should win (higher precedence). This is handled by Claude Code's rule loading order."
...
7. **Specify rules scoping.** Writing conventions: unscoped. Frontmatter spec: scoped to `clocs/**/*.md`.

8. **Consolidate to 4 phases.** Remove artificial granularity. Keep success criteria.

9. **Add one line explaining why `clocs/` at repo root doesn't interfere** with plugin component discovery.
