...
# CLocs Plugin Architecture Proposal: Devlog

## Objective

Author the foundational proposal for reworking the CLocs repo into a Claude Code plugin, as requested in `clocs-plan.md`.

## Plan

1. Read all repo content (CLAUDE.md, clocs-plan.md, devlogs/README.md, proposals/README.md).
...
4. Write the proposal following `clocs/proposals/README.md` format.
5. Write this devlog.
...
A plugin (`.claude-plugin/plugin.json`) gives namespaced commands (`/cloc:devlog`), bundled hooks and rules, and a versioned manifest.
...
4. **README removal loses plugin-less fallback.** Without READMEs, `clocs/` is opaque to non-plugin users and GitHub readers.
5. **10 phases over-decomposed** for what is primarily markdown authoring. 4 phases is more honest.

Full review: `clocs/reviews/2026-01-29-review-of-clocs-plugin-architecture.md`
...
- Rules scoping explicit: writing-conventions and workflow-patterns unscoped, frontmatter-spec scoped to `clocs/**/*.md`.
...
| File | Description |
|------|-------------|
| `clocs/proposals/2026-01-29-clocs-plugin-architecture.md` | Proposal (revised): 6 skills in 2 categories, 3 rules, hooks, 4 phases |
| `clocs/reviews/2026-01-29-review-of-clocs-plugin-architecture.md` | Self-review with 9 action items, verdict: revise |
| `clocs/devlogs/2026-01-29-clocs-plugin-proposal.md` | This devlog |

## Verification

- Proposal follows required sections from `clocs/proposals/README.md` (BLUF, objective, background, proposed solution, design decisions, edge cases, test plan, implementation phases).
- Frontmatter follows spec from `clocs-plan.md`.
- All items from `clocs-plan.md` addressed: skills, hooks, rules, CLI/status, scaffolding, marketplace (deferred), writing conventions, formatting automation (deferred), reviews/reports (research flagged).
