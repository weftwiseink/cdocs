...
task_list: clocs/plugin_architecture
type: proposal
state: live
status: review_ready
last_reviewed:
...
# CLocs Plugin Architecture

> BLUF: Package CLocs as a Claude Code plugin with six skills, shared rules, and a validation hook.
...
> Existing README guidelines are absorbed into skills, but `init` generates lightweight READMEs in each `clocs/` subdir for plugin-less discoverability.
> v1 distribution: git clone + `claude --plugin-dir`.
> Key sources: Claude Code plugin spec (`.claude-plugin/plugin.json`, skills, hooks, rules), existing `clocs/` READMEs, `clocs-plan.md`.

## Objective

Transform the current CLocs repo (a collection of markdown guidelines and directory scaffolding copied from a project) into a distributable Claude Code plugin.
The plugin should encode document creation workflows, writing conventions, and formatting enforcement so that any project can adopt CLocs by installing the plugin and running `/cloc:init`.

## Background

**Current state:**
The repo contains a `clocs/` directory with READMEs specifying devlog and proposal formats, a `CLAUDE.md` with project-wide writing and workflow directives, and a `clocs-plan.md` with design notes and future work items.
No skills, hooks, or plugin manifest exist yet.

**Claude Code extension points used by this proposal:**

| Mechanism | Role in CLocs |
|-----------|---------------|
...
| Hooks (`hooks/hooks.json`) | Formatting validation on clocs writes |
...
│   └── init/
│       └── SKILL.md                    # scaffold clocs in a project
├── hooks/
│   ├── hooks.json                      # PostToolUse validation
│   └── clocs-validate-frontmatter.sh   # validation script
├── rules/
...
│   └── frontmatter-spec.md            # field definitions + enums (scoped to clocs/)
├── clocs/                              # plugin's own dev docs (dogfooding)
│   ├── devlogs/
│   ├── proposals/
│   ├── reviews/
│   ├── reports/
...
The `clocs/` directory at repo root is the plugin's own development documentation (dogfooding).
...
# Add: "enabledPlugins": { "cloc": true }
# With plugin path configured

# Option C: Vendor into project (no external dependency)
cp -r ~/.claude/clocs-plugin/.claude-plugin .claude-plugin
cp -r ~/.claude/clocs-plugin/skills .claude/skills  # loses namespacing
cp -r ~/.claude/clocs-plugin/rules .claude/rules
```

Option A or B is recommended.
Option C loses `/cloc:` namespacing (skills become `/devlog`, `/propose`, etc.) but works without plugin infrastructure.
...
- `/cloc:devlog`: Most commonly auto-invoked by Claude when starting substantive work (triggered by the "always create a devlog" rule). User can also invoke directly.
- `/cloc:propose`: Typically user-invoked when a design needs specification. Claude may suggest it when scoping complex work.
- `/cloc:review`: Typically user-invoked. Claude may suggest it when a document reaches `review_ready`.
- `/cloc:report`: Typically user-invoked after research or analysis. Claude may suggest it after completing investigative work.
- `/cloc:status`: User-invoked for doc inventory. Claude may auto-invoke to check document state.
- `/cloc:init`: User-invoked to scaffold a new project.

#### `/cloc:devlog`: Create a development log

- **User-invocable:** yes
- **Argument hint:** `[feature_name]`
- **Expected usage:** Most commonly auto-invoked by Claude. The `rules/writing-conventions.md` rule contains the "always create a devlog when starting substantive work" directive. This rule is the *trigger*; the skill is the *mechanism*. The user can also invoke `/cloc:devlog feature-name` directly.
- **Behavior:**
  1. Determine date and feature name (from arg or prompt).
  2. Create `clocs/devlogs/YYYY-MM-DD-feature-name.md` with frontmatter and section scaffold from `template.md`.
...
- **Skill instructions absorb:** `clocs/devlogs/README.md` content (structure, best practices, verification requirements, debugging phases).

#### `/cloc:propose`: Author a proposal

- **User-invocable:** yes
- **Argument hint:** `[topic]`
- **Behavior:**
  1. Create `clocs/proposals/YYYY-MM-DD-topic.md` with frontmatter and required sections.
  2. Embed the author checklist.
  3. Guide Claude through BLUF-first drafting, design decisions, implementation phases.
- **Skill instructions absorb:** `clocs/proposals/README.md` content (required sections, implementation phase guidance, author checklist).

#### `/cloc:review`: Review a document
...
- **Behavior:**
  ...
  2. Create `clocs/reviews/YYYY-MM-DD-review-of-{doc-name}.md`.
...
#### `/cloc:report`: Generate a report

- **User-invocable:** yes
- **Argument hint:** `[topic]`
- **Behavior:**
  1. Create `clocs/reports/YYYY-MM-DD-topic.md` with frontmatter.
...
- **This is the lightweight "docs as DB" interface** referenced in `clocs-plan.md`.
  A future MCP server could replace or augment this with richer queries.
- **Scaling note:** This approach reads every clocs file to parse frontmatter.
  Practical up to ~100 documents.
  For larger corpora, a frontmatter index file (`clocs/.index.json`) maintained by the PostToolUse hook would avoid repeated full scans.
  Alternatively, promote the MCP server timeline if this becomes a bottleneck.

#### `/cloc:init`: Scaffold CLocs in a project

- **User-invocable:** yes
- **Argument hint:** `[--minimal]`
- **Behavior:**
  1. Create directory structure: `clocs/{devlogs,proposals,reviews,reports,_media}/`.
  2. Generate lightweight READMEs in each subdir with format summaries and a reference to the full skill (e.g., "See `/cloc:propose` for complete authoring guidelines"). These serve as fallback documentation for non-plugin users and GitHub readers.
  3. Create a `.claude/rules/clocs.md` rule file with core CLocs writing conventions, or append a CLocs section to the project's `CLAUDE.md`.
  4. Optionally enable the clocs plugin in `.claude/settings.json`.
...
#### `rules/frontmatter-spec.md` (scoped to `clocs/**/*.md`)
...
Scoped so the frontmatter schema only loads into context when working on clocs documents.

Extracted from `clocs-plan.md` "frontmatter" section:
  ...
  "PostToolUse": [
    {
      ...
      "hooks": [
        {
          "type": "command",
          "command": "${CLAUDE_PLUGIN_ROOT}/hooks/clocs-validate-frontmatter.sh",
          "timeout": 5
        }
      ]
    }
...
**Script location:** `hooks/clocs-validate-frontmatter.sh` in the plugin directory, resolved via `${CLAUDE_PLUGIN_ROOT}`.
...
- Exits immediately (code 0, no output) if path does not match `clocs/**/*.md`.
...
> NOTE(claude-opus-4-5/plugin_architecture): The markdown formatting automation (table alignment, diagram padding) mentioned in `clocs-plan.md` is deferred to future work.
...
| "Workflow" (conventional commits) | Stays in `CLAUDE.md` (project-specific, not CLocs) |
...
- Reference to CLocs plugin for guidelines (`@rules/writing-conventions.md`)
...
**Why:** A plugin gives us namespaced skills (`/cloc:devlog` vs `/devlog`), bundled hooks and rules, a versioned manifest for distribution, and a clean boundary between CLocs concerns and project concerns.
...
**Decision:** Canonical guidelines move into `SKILL.md` files. The `init` skill generates lightweight READMEs in each `clocs/` subdir.
...
However, removing READMEs entirely makes `clocs/` opaque to non-plugin users, GitHub readers, and team members without the plugin.
...
**Why:** A skill using `Glob`+`Read` is sufficient for scanning a `clocs/` directory up to ~100 documents.
...
2. **Plugin active but no `clocs/` directory:** Skills should detect this and prompt `init`, not fail silently.
...
4. **Plugin dogfooding:** The plugin repo itself uses CLocs for its own development. The plugin's own `clocs/` does not conflict with plugin component discovery, which only scans `skills/`, `hooks/`, and `rules/`.
5. **Rule conflicts with project CLAUDE.md:** If a project has its own writing conventions that conflict with CLocs rules, the project's rules win. Claude Code's loading precedence is: project scope > plugin scope, so project `CLAUDE.md` and `.claude/rules/` override plugin-provided rules.
...
7. **Plugin-less usage:** Team members without the plugin see `init`-generated READMEs in `clocs/` subdirs. These provide format guidance and can be followed manually. Full skill automation requires the plugin.
...
2. **Devlog auto-invocation:** Start a substantive task without explicitly invoking `/cloc:devlog` -> verify Claude creates one automatically.
3. **Frontmatter validation hook:** Write a clocs file with missing frontmatter -> verify warning appears in context. Write a non-clocs file -> verify no warning.
4. **Status skill:** Create several docs with varied states -> verify status output is accurate and filterable.
5. **Init skill:** Run in a clean directory -> verify full scaffolding and READMEs created. Run in a directory with existing `clocs/` -> verify no destructive overwrites.
6. **Rule loading:** Verify writing conventions appear for all work (unscoped). Verify frontmatter spec only appears when editing clocs files (scoped).
...
9. **Fallback READMEs:** Disable the plugin and verify `clocs/` subdirs contain usable format documentation.
...
- Write `rules/frontmatter-spec.md` with `paths: ["clocs/**/*.md"]` scoping (extracted from `clocs-plan.md`).
- Implement `skills/init/SKILL.md` with directory scaffolding and README generation.
- **Success criteria:** `claude --plugin-dir .` loads the plugin. Rules appear in ambient context. `/cloc:init` creates the directory structure with READMEs in a test project.
- **Constraints:** Do not modify existing `clocs/` content or CLAUDE.md yet.

### Phase 2: Core skills (devlog + proposal)

- Write `skills/devlog/SKILL.md` absorbing `clocs/devlogs/README.md`. Include invocation model, template, best practices, verification requirements, debugging phases.
- Write `skills/devlog/template.md` with frontmatter and section scaffold.
- Write `skills/propose/SKILL.md` absorbing `clocs/proposals/README.md`. Include author checklist, implementation phase guidance.
- Write `skills/propose/template.md`.
- **Success criteria:** `/cloc:devlog my_feature` creates a well-formed devlog. `/cloc:propose my_topic` creates a well-formed proposal. Starting substantive work without explicit invocation triggers devlog auto-creation.
...
- **Success criteria:** `/cloc:review path/to/doc.md` creates a linked review and updates the target. `/cloc:report my_topic` creates a well-formed report.
- **Depends on:** Phase 1.
...
- Write `hooks/hooks.json` and `hooks/clocs-validate-frontmatter.sh` (bash, regex-based YAML field detection, no external dependencies).
- Slim the repo's `CLAUDE.md` by removing content that migrated to rules and skills.
- Remove the original `clocs/devlogs/README.md` and `clocs/proposals/README.md` (absorbed into skills).
...
- **Success criteria:** `/cloc:status` shows accurate doc inventory with filtering. Frontmatter validation hook warns on missing fields. No duplicated content between CLAUDE.md, skills, and rules. Plugin README documents all skills and installation options.
- **Depends on:** Phases 2 and 3 (all skills exist before cleanup removes READMEs).

## Recommended Claude Tasks

```
clocs/plugin_architecture                          # top-level workstream
clocs/plugin_architecture/foundation               # Phase 1: skeleton + rules + init
clocs/plugin_architecture/core_skills              # Phase 2: devlog + proposal
clocs/plugin_architecture/research_skills          # Phase 3: review + report (research)
clocs/plugin_architecture/management_cleanup       # Phase 4: status + hooks + migration + docs
```
...
These items from `clocs-plan.md` are acknowledged but deferred:
...
- **Frontmatter index file:** `clocs/.index.json` maintained by hooks to accelerate status queries at scale.
