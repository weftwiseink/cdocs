...
last_reviewed:
  ...
  round: 2
task_list: clocs/haiku-subagent
type: proposal
state: live
...
> BLUF(mjr/clocs/haiku-subagent): Add a lightweight haiku subagent as an end-of-turn triage step that maintains clocs frontmatter/tags and recommends workflow continuations (auto-review, auto-revision).
...
> The approach is a new `/cloc:triage` skill backed by a haiku-model Task subagent, codified as a workflow pattern in `rules/workflow-patterns.md`.

## Objective

CLocs documents have a well-defined lifecycle (wip -> review_ready -> reviewed -> done) with frontmatter tracking state transitions, review status, and tags.
...
2. Recommend which cloc skill should be invoked next based on document state.
...
- **PostToolUse hook** (`hooks/clocs-validate-frontmatter.sh`): validates field presence after Write/Edit, but only checks existence, not semantic correctness.
...
A haiku-model Task subagent invoked at the end of substantive work on clocs documents.
...
**Invocation:** the top-level agent spawns a haiku Task subagent after completing substantive clocs work.
"End of turn" means: after the agent finishes responding to a user message that involved creating or modifying clocs documents.
Triage is not invoked mid-authoring or after trivial edits (typo fixes, formatting).

**Inputs (via prompt):**
- List of clocs file paths created or modified in the current turn (absolute from repo root).
...
CHANGES APPLIED:
- clocs/proposals/2026-01-29-foo.md:
  tags: added [hooks, workflow], removed [future_work]
- clocs/devlogs/2026-01-29-bar.md:
  no changes needed
...
- [REVIEW] clocs/proposals/2026-01-29-foo.md is review_ready. Recommend: spawn opus review subagent.
- [REVISE] clocs/proposals/2026-01-28-bar.md has revision_requested (round 1). Recommend: top-level agent revises inline.
- [ESCALATE] clocs/proposals/2026-01-27-baz.md has revision_requested (round 3). Recommend: escalate to user.
- [NONE] clocs/devlogs/2026-01-29-qux.md: no action needed.
```
...
NOTE(mjr/clocs/haiku-subagent): The user suggested compaction when context > 50%.
...
**Decision:** New `/cloc:triage` skill + workflow pattern in `rules/workflow-patterns.md`.
...
> TODO(mjr/clocs/haiku-subagent): A wrapping utility script for structured YAML frontmatter mutations would provide machine-oriented correctness.
...
An opus review subagent spawned via Task doesn't automatically load the clocs plugin's rules and skill instructions.
...
NOTE(mjr/clocs/haiku-subagent): This is a real constraint of the Task tool.
...
1. **Manual triage invocation**: invoke `/cloc:triage` on a known set of clocs files, verify frontmatter updates and recommendations are correct.
...
7. **Recommendation accuracy**: run triage across all existing clocs (N documents), verify recommendations are sensible and conservative (no false review_ready recommendations on incomplete documents).
...
NOTE(mjr/clocs/haiku-subagent): This was validated during dogfooding of this proposal.
...
**Success criteria:** haiku Task subagent successfully reads clocs files and returns structured output.
...
3. The skill instructs the top-level agent to spawn a haiku Task subagent with:
   - The list of modified clocs file paths.
...
**Success criteria:** `/cloc:triage` can be invoked, spawns a haiku agent, and returns a triage report.
...
2. Test on existing clocs in this repo: run triage against all documents, verify recommendations are sensible.
...
1. Add `/cloc:triage` to the skills reference in CLAUDE.md.
...
The top-level agent fills in `$FILES` with the list of modified clocs file paths.

```
You are a CLocs triage subagent. Your job is to maintain clocs frontmatter and
...
2. Check frontmatter fields against the CLocs frontmatter spec:
